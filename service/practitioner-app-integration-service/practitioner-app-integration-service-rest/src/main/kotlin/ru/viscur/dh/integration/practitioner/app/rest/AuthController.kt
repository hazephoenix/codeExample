...
import ru.viscur.dh.integration.practitioner.app.api.model.AuthInfoAppDto
import ru.viscur.dh.security.DhUserDetails
import java.lang.Exception

@RestController
@RequestMapping("/integration/practitioner-app/auth")
class AuthController(
        val practitionerAppService: PractitionerAppService
) {
     ...
     * Непосредсвенно аутентификацией занимается  [ru.viscur.dh.integration.practitioner.app.rest.security.MisAuthenticationProvider]
     * и Spring Security. Настроено все в [ru.viscur.dh.integration.practitioner.app.rest.config.PractitionerAppRestConfig].
     *
     *  + Если аутентификация прошла успешно, запрос приходит в данный метод, данный метод возвращает
     *   [AuthInfoAppDto] который показывает что пользователь прошел аутентификацию.
     *  + Если аутентификация не прошла, то Spring Security вернет 401
     */
    @GetMapping("/auth-info")
    fun authInfo(): AuthInfoAppDto {
    ...
    private fun authenticated(authentication: Authentication): AuthInfoAppDto {
        val details = authentication.details as DhUserDetails
        return AuthInfoAppDto(
